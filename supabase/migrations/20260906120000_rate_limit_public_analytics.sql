...
begin
  ...
  if p_profile is not null and not exists (select 1 from profiles where id = p_profile) then raise exception 'invalid analytics profile'; end if;
  ...
  insert into analytics_ingestion_limits(source_hash,window_start,event_count) values(p_source_hash,v_window,1)
  on conflict(source_hash,window_start) do update set event_count=analytics_ingestion_limits.event_count+1
  returning event_count into v_count;
  if v_count > 120 then raise exception 'analytics rate limit exceeded'; end if;
  insert into analytics_events(action,venue_id,event_id,profile_id,metadata) values(p_action,p_venue,p_event,p_profile,p_metadata);
end;
$$;
