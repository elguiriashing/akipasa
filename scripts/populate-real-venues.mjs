...
const targetLocalities = [
  ...
  },
  {
    ...
    lat: 43.263,
    lon: -2.935,
    niches: ["food", "music", "workshop"],
  },
  {
    slug: "carvajal",
    name: "Torreblanca / Carvajal",
    lat: 36.569,
    lon: -4.595,
    niches: ["family", "sport", "food"],
  },
  {
    slug: "fuengirola",
    name: "Fuengirola",
...
async function run() {
  try {
    ...
    for (const loc of targetLocalities) {
      ...
      const cityExists =
        await sql`SELECT id FROM cities WHERE slug = ${loc.slug}`;
    ...
    console.log(
      "Successfully populated 27 top venues and events across 9 localities.",
    );
  } catch (err) {
